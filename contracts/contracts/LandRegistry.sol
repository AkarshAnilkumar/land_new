...
contract LandRegistry {
    ...
    event DifferentPriceAlert(string name, uint256 oldPrice, uint256 newPrice);
    ...
    function addProperty(string memory _name, string memory _location, uint256 _price, address _owner) external {
        ...
        Property storage existingProperty = properties[searchLand(_name, _location)];
        if (existingProperty.price != _price) {
            emit DifferentPriceAlert(_name, existingProperty.price, _price);
        }
    ...
    function searchLand(string memory _name, string memory _location) public view returns (uint256) {
